  ...
  (with-authentication
   ...
   (lambda ()
     ...
     (define (recent-changes n)
       (map (lambda (rev)
              `(li ,(rellink `(,*admin-url* "changes" ,(car rev))
                             (caddr rev))))
            (git-rev-list "refs/heads/master" n)))
     (respond `(,(sidebar-ul `((li (h2 "posts " ,(rellink (list *admin-url* "posts")
                                                          ">>"))
                                   (ul ,@(post-links 5)))
                               (li (h2 "changes" ,(rellink (list *admin-url* "changes")
                                                           ">>"))
                                   (ul ,(recent-changes 5)))))
                (h2 "new post")
                ,(post-editing-form #f))))))
...
(define (page-admin-delete-post request body index key)
  (with-authentication
   ...
   (lambda ()
     ...
     (respond `((p "redirecting...")) #:redirect (relurl `(,*admin-url*))))))
   ...
   (lambda ()
     (let ((revs (git-rev-list (request-query-ref request "start"
       ...
       (respond `((h2 "recent changes")
                  ,@(map (lambda (rev)
                           `(div (h3 ,(rellink `(,*admin-url* "changes" ,(car rev))
                                               (caddr rev)))
                                 ,(timestamp->rfc822-date (cadr rev))))
                         revs)
                  (h3 ,(rellink (list *admin-url* "changes")
                                "more"
                                #:query
   ...
   (lambda ()
     (let ((commit (parse-commit sha1)))
       (respond `((h2 ,(assq-ref commit 'message))
                  ...
                  (form (@ (action ,(relurl `(,*admin-url* "revert-change" ,sha1)))
                           (method "POST"))
   ...
   (lambda ()
     (let ((new-master (git-revert "refs/heads/master" sha1)))
       (respond `((h3 "Change reverted"))
                #:redirect (relurl (list *admin-url*)))))))
